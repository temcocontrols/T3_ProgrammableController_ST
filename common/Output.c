...
#if ADJUST_NG2_AO	
U16_T conver_NG2_ADC(U8_T refresh_index, U16_T adc1)
{
	U16_T adc;
	if(refresh_index == 1)
	{
		adc = conver_ADC_new(new_start_adc[0],new_slop[0],adc1);
	}
	else if(refresh_index == 2)
	{
		adc = conver_ADC_new(new_start_adc[1],new_slop[1],adc1);
	}
	else
		adc = conver_ADC(adc1);
...
#if ADJUST_NG2_AO	
					adc1 = conver_NG2_ADC(refresh_index,adc1);
#else
					adc1 = conver_ADC(adc1);
#endif
				}
...
// if output is used for a digtal output, do not use feedback to adjust
		...
		{
			uint8_t channel;
			Test[27] = test_adc_flag;
			channel = (test_adc_flag >> 8);
			Test[28]++;
			Test[29] = channel;
...
void Refresh_Output(void)
{
	...
	for (;;)
	{
		...
		{	
			...
			{		
		// if block-wirte, we should do special dealwith			
			if((flag_change_realy == 1) && (count_change_relay++ < 5))
				continue;
			flag_change_realy = 0;
			count_change_relay = 0;
				...
				{							
						...
						{
							if(output_raw[i] == 1000)
							{
								temp1.word |= (0x01 << i);
							}
							else if(output_raw[i] == 0)
							{
								temp1.word &= ~(BIT0 << i); 
							}
							else 
							{							
								...
								if(pwm_count[i] < (U32_T)outputs[i].pwm_period * 20 * output_raw[i] / 1000)
								{
									temp1.word |= (0x01 << i);
								}
								else 
