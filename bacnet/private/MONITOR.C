...
//		{		// remote points	or network points
			if(get_net_point_value( ptr.pnet, &value,1,0 ) == 1)  // get rid of default value 0x55555555				
			{
		...
		{		
				...
				if(ana_dig == 1) // an
				{
					...
					if(SD_exist != 2) // inexist
					{
						...
						memcpy( PTRtable->asdu,&write_mon_point_buf[PTRtable->index * 2], MAX_MON_POINT * sizeof(Str_mon_element));
					}
					else
				...
				else  // digital
				{
					...
					if(SD_exist != 2) // inexist
					{
						...
						memcpy( PTRtable->asdu,&write_mon_point_buf[PTRtable->index * 2 + 1], MAX_MON_POINT * sizeof(Str_mon_element));									
					}
					else
...
// 0.0. --> invalid points
		if(monitors[i].inputs[j].panel != 0)
		{
			...
			{
				 // Local points
//				if(monitors[i].inputs[j].panel == panel_number) 
//				{
...
//				else
//				{ // 3VAR1 3OUT1 3IN1
//					// check whether remote modbus points is ana_dig
//					if(current_online[monitors[i].inputs[j].panel / 8] & (1 << (monitors[i].inputs[j].panel % 8)))
//					{// remote modbus points
//						uint8_t ret;
//						uint8_t index;
//						if(ret = check_remote_point_list(&monitors[i].inputs[j],&index,0))
//						{// 
