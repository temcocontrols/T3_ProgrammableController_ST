...
//				vTaskSuspend(xHandleLCD_task);
			...
			{
				U16_T send_len;
				if((pData[UIP_HEAD + 1] == READ_DIS_INPUT) || (pData[UIP_HEAD + 1] == READ_COIL))
					send_len = (pData[UIP_HEAD + 5] + 7) / 8 + 3;
				else if((pData[UIP_HEAD + 1] == READ_VARIABLES) || (pData[UIP_HEAD + 1] == READ_INPUT))
					send_len = pData[UIP_HEAD + 5] * 2 + 3;
				else
					send_len = 6; /* write responses: Unit+PDU, no RTU CRC */
				Set_transaction_ID(header, ((U16_T)pData[0] << 8) | pData[1], send_len);
			}
