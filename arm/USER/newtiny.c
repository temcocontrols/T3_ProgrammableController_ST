...
void inpust_scan(void)
{
	...
	adc_temp = 1023L * (sum - max - min) / 100 / Modbus.vcc_adc;	
	...
	if(adc_temp < 65535)
	{
		...
		//Test[20 + channel_count] = adc_temp * 300 / 1023;
...
//	if(input_type[channel_count] == INPUT_PT1K)
//			delay_ms(200);  ///only for Masood
	...
	if(Modbus.mini_type == MINI_NEW_TINY || Modbus.mini_type == MINI_TINY_ARM)
	{
		...
		{			
			if(count_show_id++ % 2 == 0)										
				memset(LED_status,0,OUT_NUM + IN_NUM);
			else
				memset(LED_status,5,OUT_NUM + IN_NUM);
...
{	
	...
	{		
		...
		if(loop_200ms++ % 10 == 0)	
		{
			...
			// check whether SW is locked
			// if all SW are SW_OFF, maybe SW is locked. need initial the SW again.
			if((outputs[0].switch_status == SW_OFF) && 
				(outputs[1].switch_status == SW_OFF) &&
				(outputs[2].switch_status == SW_OFF) &&
				(outputs[3].switch_status == SW_OFF) &&
				(outputs[4].switch_status == SW_OFF) &&
				(outputs[5].switch_status == SW_OFF) &&
				(outputs[6].switch_status == SW_OFF) &&
				(outputs[7].switch_status == SW_OFF) &&
				(outputs[8].switch_status == SW_OFF) &&
				(outputs[9].switch_status == SW_OFF) &&
				(outputs[10].switch_status == SW_OFF) &&
				(outputs[11].switch_status == SW_OFF) &&
				(outputs[12].switch_status == SW_OFF) &&
				(outputs[13].switch_status == SW_OFF))
			{
					retry_sw_init++;
					if(retry_sw_init < 5)
					{
