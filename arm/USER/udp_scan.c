...
//	conn = uip_udp_new(&addr, HTONS(UDP_SCAN_LPORT));
//#if 1// ARM_UART_DEBUG
//	printf("intial scan ok...\r\n");
//#endif
//	if(conn != NULL) 
//	{ 
...
//				else
				 memcpy(&Infor[temp_rec_scan_index++],&Scan_Infor, sizeof(STR_SCAN_CMD));
					...
					if((scan_db[i].product_model >= CUSTOMER_PRODUCT) || (current_online[scan_db[i].id / 8] & (1 << (scan_db[i].id % 8))))	  	 // in database but not on_line
					{
						...
						if(scan_db[i].sn != 
							Modbus.serialNum[0] + (U16_T)(Modbus.serialNum[1] << 8)	+ ((U32_T)Modbus.serialNum[2] << 16) + ((U32_T)Modbus.serialNum[3] << 24))
							{
								...
								if(temp_rec_scan_index < SUB_NET_MAX_COUNT)
									memcpy(&Infor[temp_rec_scan_index++],&Scan_Infor, sizeof(STR_SCAN_CMD));
...
// if id confict, send it to T3000
			...
			//if(conflict_num == 1)
			{
				...
				Scan_Infor.product = id_conflict[i].product;//0x08;//(U16_T)scan_db[i].product_model << 8;
	...
	//						if(status > 0)
							{
								...
								if(remote_panel_db[i].product_model == 0)
									Test[2]++;
   ...
   { 
		...
		//	 Modbus.ghost_gate_addr[3] = Modbus.gate_addr[3] ;
				...
				if((Modbus.ip_addr[0] != 0)   && (Modbus.ip_addr[3] != 0) )
				{		
		#if ASIX			
...
//						generatealarm(&scan_temp[4],255, pData[3], VIRTUAL_ALARM, alarm_at_all, ind_alarm_panel, alarm_panel, 0);		
      ...
      src.len = 0;		
		...
		if(temp_rec_scan_index > 0)
		{
			rec_scan_index = temp_rec_scan_index;
...
void Response_Scan_appcall(void)
{
  ...
  memset(temp_buf_scan, 0, 140);
	...
	{	
		...
		if(t1 - t2 >= 100)
		{					
			if(send_scan_index == 0)// rec_scan_index) 
			{
					...
					loop = 0; // number of mstp panel
					
					for(i = 0;i < remote_panel_num;i++)
					{	
						if(remote_panel_db[i].protocal == BAC_MSTP)
						{		
							temp_buf_scan[6 + 15 + sub_no * 2 + loop * 2] = 1;
							temp_buf_scan[6 + 15 + sub_no * 2 + loop * 2 + 1] = remote_panel_db[i].sub_id;	
							loop++;
						}
					}
					temp_buf_scan[1] = sub_no + loop;
					uip_send(temp_buf_scan, 140);
					send_scan_index++;
			}
			else if(send_scan_index < rec_scan_index + 1)
			{	
				uip_send(&Infor[send_scan_index - 1], sizeof(STR_SCAN_CMD));	
				send_scan_index++;
			}		
			t2 = uip_timer;
		}		
